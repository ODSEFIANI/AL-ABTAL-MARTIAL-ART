...
export const AiCoach: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      role: "model",
      text: "Bonjour! Je suis Coach Abtal (IA). Une question sur nos cours pour enfants ou nos horaires?",
      timestamp: Date.now(),
    },
  ]);
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      ...
      <button
        ...
        className={`${
          isOpen ? "hidden" : "flex"
        } items-center justify-center w-14 h-14 bg-accent rounded-full text-white shadow-lg hover:bg-red-700 transition transform hover:scale-110`}
      >
        <svg
          className="w-7 h-7"
          fill="none"
